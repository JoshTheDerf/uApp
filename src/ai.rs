...
- The app's UI is plain HTML/CSS/JS stored as files in the archive. `app/index.html` is the entry point, shown in an iframe; it reloads automatically whenever you write a file. The app's real runtime console output and uncaught errors (with stack traces) are captured for you — after an edit or a run_js that exercises the UI, call `read_console` to check the reload didn't throw. A `[console] …` note on a tool result means new errors are waiting; read them.
